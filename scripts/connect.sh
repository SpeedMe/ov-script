...
mysql -h$HOST -P$PORT -u$USER -p$PASS $DB -e "INSERT INTO traffic_log (log_id, node_id, email, trusted_ip, trusted_port, remote_ip, remote_port, start_time, end_time, received, send) VALUES(NULL, $NODE_ID, '$common_name','$trusted_ip', '$trusted_port','$ifconfig_pool_remote_ip', '$remote_port_1', now(),NULL, '$bytes_received', '$bytes_sent')"
...
mysql -h$HOST -P$PORT -u$USER -p$PASS $DB -e "UPDATE user SET speed_online=1, last_speed=now() WHERE email='$common_name'"
